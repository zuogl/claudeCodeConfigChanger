...
class InteractiveMenu {
    ...
    async switchToGroup(groupName) {
        ...
        try {
            ...
            const isPowerShell = this.configParser.isPowerShell;

            const baseUrlLine = group.lines.find(line => {
                const trimmed = line.trim();
                return isPowerShell ?
                    trimmed.startsWith('$env:ANTHROPIC_BASE_URL=') :
            ...
            const tokenLine = group.lines.find(line => {
                const trimmed = line.trim();
                return isPowerShell ?
                    trimmed.startsWith('$env:ANTHROPIC_AUTH_TOKEN=') :
                    trimmed.startsWith('export ANTHROPIC_AUTH_TOKEN=');
            });
